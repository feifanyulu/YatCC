...
2、类型检查和ASG生成json文件的部分，已经进行了基本的实现。同学们只要认真阅读asg.hpp文件，了解每个非终结符对应类型的结构和操作，结合上述重点参考的文法完成`par.y`文件中的**文法撰写和语义动作的补充**。
...
由于task1的标准答案是复活部分的代码输入，因此同学们需要补充`lex.cpp`文件，将clang标准输出跟语法分析的输入进行匹配。
